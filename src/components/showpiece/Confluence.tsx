...
const leftPath = (y: number) =>
  `M 190 ${y * 4.6} C 330 ${y * 4.6}, 380 ${HUB.y}, ${HUB.x - 34} ${HUB.y}`;
const rightPath = (y: number) =>
  `M 810 ${y * 4.6} C 670 ${y * 4.6}, 620 ${HUB.y}, ${HUB.x + 34} ${HUB.y}`;
...
export function Confluence({ className }: { className?: string }) {
  ...
  return (
    <div className={cn("relative", className)}>
      {/* ---------- desktop figure ---------- */}
      <div className="relative hidden aspect-1000/460 w-full sm:block">
        {/* connective tissue */}
        <svg
        ...
        {/* stream headings */}
        <span className="label absolute left-[10%] top-0 text-muted">
          Your documents
        </span>
        <span className="label absolute right-[10%] top-0 text-muted">
        ...
        {/* input chips */}
        {LEFT.map((c) => (
          <Chip key={c.label} style={{ left: "10%", top: `${c.y}%` }} align="left">
            {c.label}
          </Chip>
        ))}
        {RIGHT.map((c) => (
          <Chip key={c.label} style={{ right: "10%", top: `${c.y}%` }} align="right">
            {c.label}
          </Chip>
        ))}
        ...
        <div
          ...
          style={{ top: "45.6%" }}
        >
          <div className="relative grid h-[76px] w-[76px] place-items-center rounded-2xl bg-petrol-800 text-cream shadow-[0_16px_40px_-12px_rgba(47,42,155,0.55)]">
      ...
      {/* ---------- phone figure: same story, stacked ---------- */}
  ...
  return (
    <span
      className={cn(
        ...
        align === "left" ? "-translate-x-full" : "translate-x-full",
      )}
      style={style}
    >
      {children}
